...
```sh
$ npm install -g sogou-changelog
$ cd my-project
$ sogou-changelog -p angular -i CHANGELOG.md -s
```
...
```sh
$ sogou-changelog -p angular -i CHANGELOG.md -s -r 0
```

This *will* overwrite any previous changelog if exist.

All available command line parameters can be listed using CLI: `sogou-changelog --help`.

**Hint:** You can alias your command or add it to your package.json. EG: `"changelog": "sogou-changelog -p angular -i CHANGELOG.md -s -r 0"`.
...
{
  "scripts": {
    "version": "sogou-changelog -p angular -i CHANGELOG.md -s -r 0 && git add CHANGELOG.md"
  }
}
```
